...
class MMA(Optimizer):
    ...
    GPU version of MMA using CuPy arrays. Identical API to CPU version but operates
    on GPU memory for maximum performance in large-scale topology optimization.
    
    Parameters
    ----------
    problem : Problem
        Optimization problem (e.g., MinimumCompliance) with CUDA backend
