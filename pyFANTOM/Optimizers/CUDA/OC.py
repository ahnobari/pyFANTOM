...
class OC(Optimizer):
    ...
    GPU version of OC using CuPy arrays. Fast heuristic optimizer specifically designed
    for topology optimization with independent volume constraints. Identical API to CPU version.
    
    Parameters
    ----------
    problem : Problem
        Optimization problem (must have independent constraints) with CUDA backend
    move : float, optional
